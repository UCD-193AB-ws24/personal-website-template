...
function DraftLoader({
	...
	useEffect(() => {
		...
		fetchDraftName(parseInt(draftNumber!)).then((name) =>
			setDraftName(name)
		);
...
export default function Editor() {
	...
	useEffect(() => {
		if (typeof window !== 'undefined') {
			...
			setEditorHeight(
				lowestY > defaultHeight ? lowestY + 100 : defaultHeight
			);
		}
	}, [components]);
	...
	const handleDragEnd = ({ active, over }: DragEndEvent) => {
      ...
      const newSize = componentSizes[activeComponent.type] || { width: draggedRect.width, height: draggedRect.height };
