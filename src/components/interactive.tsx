...
export default function Interactive({
  ...
  const ref =
    useComponentVisibleInteractive<HTMLDivElement>(handleClickOutside);
  ...
  const handleResize2D = (e: React.MouseEvent) => {
    ...
    const centerPoint = {
      x: childPos.x + childSize.x / 2,
      y: childPos.y + childSize.y / 2,
    };
    ...
    const oppCornerPoint = {
      x: childPos.x + childSize.x,
      y: childPos.y + childSize.y,
    };
    ...
    const handleResizeMove = (e: MouseEvent) => {
      ...
      if (
        (oppCornerPoint.x - startPoint.x) * (oppCornerPoint.x - currentPos.x) >
        0
      ) {
        if (startPoint.x <= centerPoint.x) {
      ...
      if (
        (oppCornerPoint.y - startPoint.y) * (oppCornerPoint.y - currentPos.y) >
        0
      ) {
        if (startPoint.y <= centerPoint.y) {
  ...
  return (
    <div
      ref={ref}
      style={{
        width: `${childSize.x}px`,
        height: `${childSize.y}px`,
        position: "fixed",
        left: `${childPos.x}px`,
        top: `${childPos.y}px`,
      }}
    ...
    >
      <button
        ...
        style={{
          border: "none",
          borderRadius: "50%",
          width: `${resizeBtnWidth}px`,
          height: `${resizeBtnHeight}px`,
          position: "absolute",
          left: "-3.5px",
          top: "-3.5px",
          backgroundColor: `${resizeBtnColor}`,
          cursor: "nw-resize",
          zIndex: "10",
        }}
